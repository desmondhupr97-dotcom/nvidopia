...
export async function startIssueReportConsumer(brokers: string[]): Promise<void> {
  const kafka = new Kafka({ clientId: 'issue-workflow', brokers, retry: { retries: MAX_RETRIES } });
    ...
    eachMessage: async (payload: EachMessagePayload) => {
      ...
      while (attempt < MAX_RETRIES) {
        try {
          ...
          if (!value) { console.warn(`[issue-report-consumer] Empty message on ${topic}:${partition}`); return; }

          const event = JSON.parse(value);
          const issueId = event.issue_id ?? `ISS-${crypto.randomUUID().slice(0, 8)}`;

          const issue = await Issue.create({
            issue_id: issueId, run_id: event.run_id,
            trigger_timestamp: event.trigger_timestamp ?? new Date(),
            gps_coordinates: event.gps_coordinates,
            category: event.category ?? 'Other', severity: event.severity ?? 'Medium',
            takeover_type: event.takeover_type, data_snapshot_url: event.data_snapshot_url,
            environment_tags: event.environment_tags, description: event.description,
            status: 'New',
          });
          ...
          attempt++;
          console.error(`[issue-report-consumer] Attempt ${attempt}/${MAX_RETRIES} failed:`, err instanceof Error ? err.message : err);
          if (attempt >= MAX_RETRIES) {
            console.error(`[issue-report-consumer] Exhausted retries for message on ${topic}:${partition}, offset ${message.offset}`);
          }
        }
      }
    },
