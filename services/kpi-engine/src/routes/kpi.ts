...
// ---------------------------------------------------------------------------
// GET /kpi/mpi — Miles Per Intervention
// ---------------------------------------------------------------------------
router.get('/mpi', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[KPI] MPI error:', err);
    return res.status(500).json({ error: 'Internal server error' });
  }
});

// ---------------------------------------------------------------------------
// GET /kpi/mttr — Mean Time To Resolution
// ---------------------------------------------------------------------------
router.get('/mttr', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[KPI] MTTR error:', err);
    return res.status(500).json({ error: 'Internal server error' });
  }
});

// ---------------------------------------------------------------------------
// GET /kpi/regression-pass-rate
// ---------------------------------------------------------------------------
router.get('/regression-pass-rate', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[KPI] RegressionPassRate error:', err);
    return res.status(500).json({ error: 'Internal server error' });
  }
});

// ---------------------------------------------------------------------------
// GET /kpi/fleet-utilization
// ---------------------------------------------------------------------------
router.get('/fleet-utilization', async (req: Request, res: Response) => {
  try {
    const { project_id, start_date, end_date } = req.query;
    if (!project_id) {
      return res.status(400).json({ error: 'project_id is required' });
    ...
    console.error('[KPI] FleetUtilization error:', err);
    return res.status(500).json({ error: 'Internal server error' });
  }
});

// ---------------------------------------------------------------------------
// GET /kpi/issue-convergence
// ---------------------------------------------------------------------------
router.get('/issue-convergence', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[KPI] IssueConvergence error:', err);
    return res.status(500).json({ error: 'Internal server error' });
  }
});
